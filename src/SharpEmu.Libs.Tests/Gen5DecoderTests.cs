...
public sealed class Gen5DecoderTests
{
    ...
    [Fact]
    public void CompilesPackedByteConversionWithDefinedFloatBoundsToSpirv()
    {
        // Encoding assembled with LLVM 18 llvm-mc for gfx1030 and verified
        // with llvm-objdump.
        var ctx = CreateContext(
        [
            0xD55E0000u, 0x040E0501u, // v_cvt_pk_u8_f32 v0, v1, v2, v3
            SEndpgm,
        ]);
        Assert.True(
            Gen5ShaderTranslator.TryDecodeProgram(
                ...
                out var decodeError),
            decodeError);
        Assert.Equal(["VCvtPkU8F32", "SEndpgm"],
            program.Instructions.Select(instruction => instruction.Opcode));
        ...
        Assert.True(
            Gen5SpirvTranslator.TryCompileComputeShader(
                ...
                out var compileError),
            compileError);
        Assert.Equal(1, CountSpirvOpcode(shader.Spirv, (ushort)SpirvOp.IsNan));
        Assert.Equal(
            1,
            CountSpirvOpcode(shader.Spirv, (ushort)SpirvOp.FOrdLessThanEqual));
        Assert.Equal(
    ...
    [Fact]
    public void CompilesImageGatherBiasAndLodVariantsToSpirv()
    {
        var ctx = CreateContext(
        [
            0xF1140108u, 0x00002000u, // image_gather4_b v[32:35], v[0:2]
            0xF1100108u, 0x00002000u, // image_gather4_l v[32:35], v[0:2]
            0xF1340108u, 0x00002000u, // image_gather4_c_b v[32:35], v[0:3]
            0xF1300108u, 0x00002000u, // image_gather4_c_l v[32:35], v[0:3]
            0xF1540108u, 0x00002000u, // image_gather4_b_o v[32:35], v[0:3]
            0xF1500108u, 0x00002000u, // image_gather4_l_o v[32:35], v[0:3]
            SEndpgm,
        ]);
        Assert.True(
            Gen5ShaderTranslator.TryDecodeProgram(
        ...
        Assert.Equal(
            ...
            program.Instructions.Take(6).Select(instruction => instruction.Opcode));

        var scalarRegisters = new uint[128];
        var state = new Gen5ShaderState(program, [], Metadata: null);
        var imageBindings = program.Instructions.Take(6)
            .Select(
                instruction => new Gen5ImageBinding(
                    instruction.Pc,
                    instruction.Opcode,
        ...
        var gathers = GetSpirvInstructions(shader.Spirv, SpirvOp.ImageGather);
        Assert.Equal(6, gathers.Count);

        // Bias gathers: 6 base + image_operands + bias = 8 words
        Assert.Equal(8u, gathers[0][0] >> 16);
        Assert.Equal(0x01u, gathers[0][6]);
        Assert.Equal(8u, gathers[2][0] >> 16);
        Assert.Equal(0x01u, gathers[2][6]);

        // LOD gathers: 6 base + image_operands + lod = 8 words
        Assert.Equal(8u, gathers[1][0] >> 16);
        Assert.Equal(0x02u, gathers[1][6]);
        Assert.Equal(8u, gathers[3][0] >> 16);
        Assert.Equal(0x02u, gathers[3][6]);

        // Bias+offset gathers: 6 base + image_operands + bias + offset = 9 words
        Assert.Equal(9u, gathers[4][0] >> 16);
        Assert.Equal(0x11u, gathers[4][6]);
        // LOD+offset gathers: 6 base + image_operands + lod + offset = 9 words
        Assert.Equal(9u, gathers[5][0] >> 16);
        Assert.Equal(0x12u, gathers[5][6]);
    }

    [Theory]
    [InlineData(0x41u, "ImageGather4Cl", "image gather LOD clamp")]
    [InlineData(0x4Eu, "ImageGather4CBCl", "image gather LOD clamp")]
    [InlineData(0x51u, "ImageGather4ClO", "image gather LOD clamp")]
    ...
    public void CompilesBufferSubwordMemoryOperationsToSpirv()
    {
        ...
        // the access crosses the boundary between adjacent storage dwords.
        var ctx = CreateContext(
        [
            0xE0200000u, 0x80000800u, // buffer_load_ubyte v8, off, s[0:3], 0 offset:0
            0xE0240001u, 0x80000900u, // buffer_load_sbyte v9, off, s[0:3], 0 offset:1
            0xE0280002u, 0x80000A00u, // buffer_load_ushort v10, off, s[0:3], 0 offset:2
            0xE02C0003u, 0x80000B00u, // buffer_load_sshort v11, off, s[0:3], 0 offset:3
            0xE0600001u, 0x80000C00u, // buffer_store_byte v12, off, s[0:3], 0 offset:1
            0xE0680003u, 0x80000D00u, // buffer_store_short v13, off, s[0:3], 0 offset:3
            SEndpgm,
        ]);
        Assert.True(
            Gen5ShaderTranslator.TryDecodeProgram(
    ...
    [Fact]
    public void CompilesCompressedIntegerExportsWithDefinedFloatBoundsToSpirv()
    {
        var ctx = CreateContext(
        [
            0xF8001C0Fu, 0x00000100u, // exp mrt0 v0, v1, v0, v0 compr done vm
            SEndpgm,
        ]);
        Assert.True(
            Gen5ShaderTranslator.TryDecodeProgram(
                ...
                out var decodeError),
            decodeError);
        var export = Assert.IsType<Gen5ExportControl>(program.Instructions[0].Control);
        Assert.Equal("Exp", program.Instructions[0].Opcode);
        Assert.Equal(0u, export.Target);
        Assert.Equal(0xFu, export.EnableMask);
        Assert.True(export.Compressed);
        Assert.True(export.Done);
        Assert.True(export.ValidMask);
        ...
        Assert.True(
            Gen5SpirvTranslator.TryCompilePixelShader(
                ...
                out var signedShader,
                out var signedError),
            signedError);

        foreach (var shader in new[] { unsignedShader, signedShader })
        {
            Assert.Equal(4, CountSpirvOpcode(shader.Spirv, (ushort)SpirvOp.IsNan));
            Assert.Equal(
                4,
        ...
        Assert.True(ContainsSpirvOpcode(
            unsignedShader.Spirv,
            (ushort)SpirvOp.ConvertFToU));
        Assert.True(ContainsSpirvOpcode(
            signedShader.Spirv,
            (ushort)SpirvOp.ConvertFToS));
    }
    ...
    [Fact]
    public void CompilesCompactVectorFloat16SubtractionToSpirv()
    {
        var ctx = CreateContext(
        [
            0x66000501u, // v_sub_f16_e32 v0, v1, v2
            0x68060B04u, // v_subrev_f16_e32 v3, v4, v5
            SEndpgm,
        ]);
        Assert.True(
            Gen5ShaderTranslator.TryDecodeProgram(
    ...
    [Fact]
    public void CompilesCompactVectorFloat16MinMaxToSpirv()
    {
        var ctx = CreateContext(
        [
            0x72000501u, // v_max_f16_e32 v0, v1, v2
            0x74060B04u, // v_min_f16_e32 v3, v4, v5
            SEndpgm,
        ]);
        Assert.True(
            Gen5ShaderTranslator.TryDecodeProgram(
