...
public static class Gen5ShaderTranslator
{
    public static ulong[] ComputeConsumedScalarMask(Gen5ShaderProgram program)
    {
        ...
        foreach (var instruction in program.Instructions)
        {
            ...
            if (instruction.Encoding is Gen5ShaderEncoding.Smem or
                Gen5ShaderEncoding.Smrd &&
    ...
    private static void AddConsumedScalar(
        ulong[] mask,
        uint register,
        uint count)
    {
    ...
    public static bool IsScalarConsumed(
        ulong[] mask,
        uint register) =>
        register < 256 &&
        (mask[register >> 6] &
        ...
        out string error)
    {
        ...
        {
            if (!TryReadProgramWord(ctx, address, pc, 0, out var word))
            {
            ...
            {
                var wordOffset = wordIndex * sizeof(uint);
                if (!TryReadProgramWord(ctx, address, pc, wordOffset, out words[wordIndex]))
                {
                    error = $"read-failed pc=0x{pc + wordOffset:X}";
                    return false;
                }
            }
    ...
    private static bool TryReadProgramWord(
        CpuContext ctx,
        ulong baseAddress,
        uint pc,
        uint byteOffset,
        out uint word)
    {
        if (!GuestAddress.TryAdd(pc, byteOffset, out var programOffset) ||
            !GuestAddress.TryAdd(baseAddress, programOffset, out var wordAddress))
        {
            word = 0;
            return false;
        }

        return ctx.TryReadUInt32(wordAddress, out word);
    }
        ...
        out string error)
    {
        ...
        switch (word >> 26)
        {
            case 0x33:
                ...
                if (!TryReadProgramWord(ctx, baseAddress, pc, sizeof(uint), out var vop3pExtra))
                {
            ...
            case 0x35:
                ...
                if (!TryReadProgramWord(ctx, baseAddress, pc, sizeof(uint), out var vop3Extra))
                {
            ...
            case 0x38:
                ...
                if (!TryReadProgramWord(ctx, baseAddress, pc, sizeof(uint), out var mubufExtra))
                {
            ...
            case 0x3A:
                ...
                if (!TryReadProgramWord(ctx, baseAddress, pc, sizeof(uint), out var mtbufExtra))
                {
        ...
        out string error)
    {
        ...
        name = opcode switch
        {
            ...
            0x2C => "DsMskorRtnB32",
            0x2D => "DsWrxchgRtnB32",
            0x2E => "DsWrxchg2RtnB32",
            0x2F => "DsWrxchg2St64RtnB32",
            0x30 => "DsCmpstRtnB32",
            0x31 => "DsCmpstRtnF32",
            0x32 => "DsMinRtnF32",
            0x33 => "DsMaxRtnF32",
    ...
    private static bool DecodeMimg(uint word, out string name, out uint sizeDwords, out string error)
    {
        ...
        name = opcode switch
        {
            ...
            0x54 => "ImageGather4LO",
            0x55 => "ImageGather4BO",
            0x56 => "ImageGather4BClO",
            0x57 => "ImageGather4LzO",
            0x58 => "ImageGather4CO",
            0x59 => "ImageGather4CClO",
            0x5C => "ImageGather4CLO",
            0x5D => "ImageGather4CBO",
            0x5E => "ImageGather4CBClO",
        ...
        switch (encoding)
        {
            ...
            case Gen5ShaderEncoding.Ds:
            {
                ...
                sources = opcode switch
                {
                    ...
                    "DsRsubU32" or "DsIncU32" or "DsDecU32" or
                    "DsMinI32" or "DsMaxI32" or "DsMinU32" or "DsMaxU32" or
                    "DsAndB32" or "DsOrB32" or "DsXorB32" or
                    "DsMinF32" or "DsMaxF32" or "DsAddF32" or
                    "DsAddRtnU32" or "DsSubRtnU32" or
                    "DsRsubRtnU32" or "DsIncRtnU32" or "DsDecRtnU32" or
                    "DsMinRtnI32" or "DsMaxRtnI32" or
                    "DsMinRtnU32" or "DsMaxRtnU32" or
                    "DsAndRtnB32" or "DsOrRtnB32" or "DsXorRtnB32" or
                    "DsWrxchgRtnB32" or
                    "DsMinRtnF32" or "DsMaxRtnF32" or "DsAddRtnF32" => [
                        Gen5Operand.Vector(vectorAddress),
                        Gen5Operand.Vector(vectorData0),
                    ],
                    "DsMskorB32" or "DsMskorRtnB32" or
                    "DsWrapRtnB32" => [
                        Gen5Operand.Vector(vectorAddress),
                        Gen5Operand.Vector(vectorData0),
                        Gen5Operand.Vector(vectorData1),
                    ],
                    "DsWrxchg2RtnB32" or "DsWrxchg2St64RtnB32" => [
                        Gen5Operand.Vector(vectorAddress),
                        Gen5Operand.Vector(vectorData0),
                        Gen5Operand.Vector(vectorData1),
                    ],
                    "DsCmpstB32" or "DsCmpstRtnB32" or
                    "DsCmpstF32" or "DsCmpstRtnF32" => [
                        Gen5Operand.Vector(vectorAddress),
                        Gen5Operand.Vector(vectorData0),
                        Gen5Operand.Vector(vectorData1),
                    ],
                ...
                destinations = opcode switch
                {
                    ...
                    "DsMinRtnU32" or "DsMaxRtnU32" or
                    "DsAndRtnB32" or "DsOrRtnB32" or "DsXorRtnB32" or
                    "DsWrxchgRtnB32" or "DsMskorRtnB32" or
                    "DsCmpstRtnB32" or "DsWrapRtnB32" or
                    "DsCmpstRtnF32" or "DsMinRtnF32" or
                    "DsMaxRtnF32" or "DsAddRtnF32" => [
                        Gen5Operand.Vector(vectorDestination),
                    ],
                    "DsWrxchg2RtnB32" or "DsWrxchg2St64RtnB32" => [
                        Gen5Operand.Vector(vectorDestination),
