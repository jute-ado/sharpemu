...
/// name fallback, and library default defined by the export attribute contract. NIDs
...
public sealed class SysAbiExportGenerator : IIncrementalGenerator
{
    ...
    private const string DefaultGeneratedNamespace = "SharpEmu.Generated";
    private const string GeneratedNamespaceProperty = "build_property.SharpEmuGeneratedNamespace";
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var generatedNamespace = context.AnalyzerConfigOptionsProvider
            .Select(static (options, _) =>
                options.GlobalOptions.TryGetValue(GeneratedNamespaceProperty, out var configured) &&
                !string.IsNullOrWhiteSpace(configured)
                    ? configured
                    : DefaultGeneratedNamespace);

        context.RegisterSourceOutput(
            exports.Combine(assemblyName).Combine(generatedNamespace),
            static (productionContext, source) =>
                Emit(productionContext, source.Left.Left!, source.Left.Right, source.Right));
    }
        ...
        string generatedNamespace)
    {
        ...
        builder.AppendLine("    /// export contract: an attribute Target of None inherits the");
