...
public static class VideoOutExports
{
    ...
    public static void ConfigureApplicationInfo(
        string? title,
        string? titleId,
        string? version,
        string? commitSha = null)
    {
        ...
        lock (_stateGate)
        {
            var commitSuffix = string.IsNullOrWhiteSpace(commitSha)
                ? string.Empty
                : $" ({commitSha.Trim()})";
            _windowTitle = $"SharpEmu - {application}{versionSuffix}{commitSuffix}";
        }
    }
    ...
    public static int VideoOutIsOutputSupported(CpuContext ctx)
    {
        ...
        return EvaluateOutputSupport(ctx, port, out _);
    }

    private static int EvaluateOutputSupport(
        CpuContext ctx,
        VideoOutPortState port,
        out ulong mode)
    {
        mode = ctx[CpuRegister.Rsi];
    ...
    public static int VideoOutConfigureOutput(CpuContext ctx)
    {
        ...
        var support = EvaluateOutputSupport(ctx, port, out var mode);
        if (support < 0)
        {
            return support;
        }
        ...
        lock (_stateGate)
        {
            ...
            outputModeEvents = new FlipEventRegistration[port.OutputModeEvents.Count];
            port.OutputModeEvents.CopyTo(outputModeEvents);
        }

        var eventHint = mode << 16;
        for (var index = 0; index < outputModeEvents.Length; index++)
        {
            _ = KernelEventQueueCompatExports.TriggerDisplayEvent(
                outputModeEvents[index].Equeue,
                SceVideoOutInternalEventOutputMode,
                OrbisKernelEventFilterVideoOut,
                eventHint,
                outputModeEvents[index].UserData);
        }

        TraceVideoOut(
            $"videoout.configure_output handle={handle} mode=0x{mode:X16} events={outputModeEvents.Length}");
        return (int)OrbisGen2Result.ORBIS_GEN2_OK;
    }
    ...
    public static int VideoOutAddOutputModeEvent(CpuContext ctx)
    {
        ...
        lock (_stateGate)
        {
            ...
            if (existingIndex >= 0)
            {
                port.OutputModeEvents[existingIndex] =
                    new FlipEventRegistration(equeue, userData);
            }
            else
            {
                port.OutputModeEvents.Add(
                    new FlipEventRegistration(equeue, userData));
            }
        ...
        TraceVideoOut(
            ...
            $"udata=0x{userData:X16} mode=0x{mode:X16}");
        return (int)OrbisGen2Result.ORBIS_GEN2_OK;
    }
    ...
    public static int VideoOutGetEventId(CpuContext ctx)
    {
        ...
        return ident is SceVideoOutInternalEventFlip or
            SceVideoOutInternalEventVblank or
            SceVideoOutInternalEventOutputMode
            ? (int)OrbisGen2Result.ORBIS_GEN2_OK
    ...
    public static int VideoOutRegisterBuffers2(CpuContext ctx)
    {
        ...
        // upper 32 bits of the slot are stale (games leave GNM magic there).
        ...
        int requestedGroupIndex = -1)
    {
        lock (_stateGate)
        {
            ...
            TraceVideoOut(
                ...
                $"fmt=0x{attribute.PixelFormat:X} tile={attribute.TilingMode} {attribute.Width}x{attribute.Height} " +
                $"pitch={attribute.PitchInPixel} compression={compression}");
    ...
    internal static bool TryReadBufferAttribute(CpuContext ctx, ulong attributeAddress, bool attribute2, out BufferAttribute attribute)
    {
        attribute = default;
        ...
        {
            var option =
                BinaryPrimitives.ReadUInt64LittleEndian(structure[0x18..0x20]);
            var pixelFormat =
                BinaryPrimitives.ReadUInt64LittleEndian(structure[0x20..0x28]);
            var dccClearColor =
                BinaryPrimitives.ReadUInt64LittleEndian(structure[0x28..0x30]);
            var dccControl =
                BinaryPrimitives.ReadUInt32LittleEndian(structure[0x30..0x34]);
        ...
        var pixelFormat32 =
            BinaryPrimitives.ReadUInt32LittleEndian(structure[0x00..0x04]);
        var aspectRatio =
            BinaryPrimitives.ReadUInt32LittleEndian(structure[0x08..0x0C]);
        var pitchInPixel =
            BinaryPrimitives.ReadUInt32LittleEndian(structure[0x14..0x18]);
        var option32 =
            BinaryPrimitives.ReadUInt32LittleEndian(structure[0x18..0x1C]);
