...
internal static unsafe class VulkanVideoPresenter
{
    ...
    internal static VulkanTextureImageShape ResolveTextureImageShape(
        ...
        LoadGuestImageWriteCaptureRequest()
    {
        ...
        if (GuestImageWriteCaptureRequest.TryParse(value, out var request))
        {
            return request;
        }
    ...
    internal static void ProvideGuestImageInitialData(ulong address, byte[] rgbaPixels)
    {
        ...
        if (address == 0 || rgbaPixels.Length == 0)
        {
            return;
        }
    ...
    private static bool HasReadyPresentation(long presentedSequence)
    {
        lock (_gate)
        {
            ...
            {
                var pending = _pendingGuestImagePresentations.Peek();
                return pending.Sequence > presentedSequence &&
    ...
    private sealed class Presenter : IDisposable
    {
        ...
        private void SubmitGuestCommandBufferAndWait(
            CommandBuffer commandBuffer)
        {
            var fence = AcquireGuestFence();
            var submitted = false;
            try
            {
                ...
                Check(
                    _vk.WaitForFences(
                        ...
                        true,
                        ulong.MaxValue),
                    "vkWaitForFences(guest readback)");
            }
            finally
            {
                ReleaseGuestFence(fence, needsReset: submitted);
            ...
            GuestDepthResource? feedbackDepth = null)
        {
            ...
            var resources = new TranslatedDrawResources
            {
                ...
                DepthAttachmentFormat =
                    feedbackDepth?.AttachmentFormat ?? Format.Undefined,
            ...
            try
            {
                ...
                fixed (VertexInputAttributeDescription* vertexAttributePointerBase = vertexAttributeDescriptions)
                {
                    ...
                    var depthStencil = new PipelineDepthStencilStateCreateInfo
                    {
                        ...
                        StencilTestEnable = depth.Stencil.TestEnable,
                        Front = CreateStencilOpState(depth.Stencil.Front),
                        Back = CreateStencilOpState(depth.Stencil.Back),
                    };
                    var pipelineInfo = new GraphicsPipelineCreateInfo
                    {
                        SType = StructureType.GraphicsPipelineCreateInfo,
        ...
        private static bool IsCompatibleGuestImageAlias(
            ...
            GuestImageResource guestImage)
        {
            if (guestImage.IsThreeDimensional != texture.ThreeDimensionalView ||
                (texture.ThreeDimensionalView &&
                 guestImage.Depth != Math.Max(texture.Depth, 1)))
            {
                return false;
            }
        ...
        private GuestImageResource ResolveStorageGuestImage(GuestDrawTexture texture)
        {
            ...
            var guestImage = GetOrCreateGuestImage(
                ...
                depth: texture.ThreeDimensionalView
                    ? Math.Max(texture.Depth, 1)
                    : 1,
                threeDimensional: texture.ThreeDimensionalView);
            var selectedMipLevel = GetStorageMipLevel(texture);
            ...
            GuestDepthResource source)
        {
            ...
            try
            {
                var imageInfo = new ImageCreateInfo
                {
                    SType = StructureType.ImageCreateInfo,
                    ImageType = ImageType.Type2D,
                    Format = source.AttachmentFormat,
                    Extent = new Extent3D(source.Width, source.Height, 1),
                    MipLevels = 1,
                    ArrayLayers = 1,
                    Samples = SampleCountFlags.Count1Bit,
            ...
            IReadOnlyList<GuestMemoryBuffer> buffers)
        {
            ...
            var ranges = new List<GuestBufferRange>(buffers.Count);
            foreach (var buffer in buffers)
            {
        ...
        {
            if (!TryGetStencilOp(face.FailOp, out var failOp) ||
                !TryGetStencilOp(face.PassOp, out var passOp) ||
                !TryGetStencilOp(face.DepthFailOp, out var depthFailOp))
            {
                throw new InvalidOperationException(
        ...
        private void WriteBackAllDirtyGuestBuffers(string? queueName = null)
        {
            ...
            foreach (var allocation in _guestBufferAllocations)
            {
                for (var index = allocation.DirtyRanges.Count - 1; index >= 0; index--)
                {
                    ...
                    allocation.DirtyRanges.RemoveAt(index);
                    var writeSucceeded = GuestBufferWriteback.TryPublishChanges(
                        memory,
                        guestAddress,
                        shadowBytes,
                        mappedBytes,
                    ...
                    if (!writeSucceeded)
                    {
                        // Successful subranges updated the shadow already, so
                        // retrying the potential range only revisits bytes that
                        // still differ.
                        MarkGuestBufferDirty(
                            allocation,
                            range.Offset,
            ...
            try
            {
                    ...
                    work.DepthTarget is { } depthTarget)
                {
                    ...
                    depth = GetOrCreateGuestDepth(
                        ...
                        loadInitialDepth: !clearDepthForDraw,
                        loadInitialStencil:
                            effectiveDepthTarget.HasStencil &&
                            !draw.RenderState.Depth.Stencil.ClearEnable);
                    ...
                    depth.ClearStencil = effectiveDepthTarget.HasStencil
                        ? effectiveDepthTarget.ClearStencil
                        : 0;
                ...
                if (depth is not null || targets.Length > 1)
                {
                    ...
                    (renderPass, framebuffer) = CreateRenderPassAndFramebuffer(
                        ...
                        attachedDepth,
                        clearDepthForDraw && !clearDepthSeparately,
                        draw.RenderState.Depth.Stencil);
                    ...
                    depth.Layout == ImageLayout.ShaderReadOnlyOptimal)
                {
                    var toDepthAttachment = new ImageMemoryBarrier
                    {
                        ...
                        Image = depth.Image,
                        SubresourceRange =
                            DepthSubresourceRange(depth.HasStencil),
                    };
                    _vk.CmdPipelineBarrier(
                        _commandBuffer,
                        PipelineStageFlags.FragmentShaderBit |
                ...
                foreach (var target in targets)
                {
                        ...
                        captureThisWrite)
                    {
                        ...
                        var shouldTraceWrite = captureThisWrite ||
                            tracePixelSpirv || traceTitleDraw
                            ? true
        ...
        {
            depth = threeDimensional ? Math.Max(depth, 1) : 1;
            if (threeDimensional && !requiresStorage)
            {
                throw new InvalidOperationException(
                    "3D guest images cannot be promoted to render targets.");
            }
            ...
            if (_guestImages.TryGetValue(target.Address, out var existing))
            {
                if (existing.LogicalWidth == target.Width &&
                    ...
                    existing.IsThreeDimensional == threeDimensional &&
            ...
            var imageInfo = new ImageCreateInfo
            {
                ...
                Flags = ImageCreateFlags.CreateMutableFormatBit |
                    ImageCreateFlags.CreateExtendedUsageBit,
                ImageType = threeDimensional
                    ? ImageType.Type3D
                    : ImageType.Type2D,
                Format = format,
                Extent = new Extent3D(physicalWidth, physicalHeight, depth),
                MipLevels = mipLevels,
                ArrayLayers = 1,
                Samples = SampleCountFlags.Count1Bit,
                Tiling = ImageTiling.Optimal,
            ...
            uint height)
        {
            var load = CreateRenderPassAndFramebuffer(
                [format], [attachmentView], width, height, [true], null, false, default);
            var initial = CreateRenderPassAndFramebuffer(
                [format], [attachmentView], width, height, [false], null, false, default);
        ...
        private (RenderPass RenderPass, Framebuffer Framebuffer) CreateRenderPassAndFramebuffer(
            ...
            bool clearDepth,
            GuestStencilState stencilState)
        {
            if (formats.Count == 0 ||
            ...
            bool loadInitialStencil)
        {
            ...
            if (_guestDepthImages.TryGetValue(key, out var existing))
            {
                ...
                TryStageInitialDepth(existing, target, loadInitialDepth);
                TryStageInitialStencil(existing, target, loadInitialStencil);
                return existing;
            }
            ...
            SetDebugName(
                ...
                $"SharpEmu guest depth attachment 0x{target.Address:X16}");
            SetDebugName(
                ObjectType.ImageView,
                sampledDepthView.Handle,
                $"SharpEmu guest sampled depth 0x{target.Address:X16}");
            ...
            bool loadInitialContents)
        {
            ...
            resource.NeedsUpload = true;
            resource.InitializationSource = "guest-memory";
        }
        ...
        private GuestRenderTarget GetDepthOnlyColorTarget(GuestDepthTarget depth)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"unsupported guest depth format {depth.GuestFormat}");
            }
            var key = new GuestDepthKey(
                depth.Address,
                depth.ReadAddress,
                ...
                ResolveGuestDepthAttachmentFormat(
                    formatPolicy,
                    depth.HasStencil));
        ...
        private void RenderCore()
        {
            ...
            var renderWorkDeadline = _renderWorkBudgetTicks > 0
                ...
                : long.MaxValue;
            while (GuestWorkPresentationPolicy.ShouldProcessGuestWork(
            ...
            ulong fingerprint = 0)
        {
                ...
                TryConvertGuestImageToRgb(image, bytes, out var rgb))
            {
                ...
                var basePath = Path.Combine(
                    ...
                    $"0x{fingerprint:X16}-{image.Width}x{image.Height}");
                RgbBitmapWriter.Write(
                    basePath + ".bmp",
                    image.Width,
                    image.Height,
            ...
            PipelineStageFlags shaderStage)
        {
            ...
            if (!depth.Initialized)
            {
                var depthRange = DepthSubresourceRange(depth.HasStencil);
                var toTransfer = new ImageMemoryBarrier
                {
                    SType = StructureType.ImageMemoryBarrier,
                    DstAccessMask = AccessFlags.TransferWriteBit,
            ...
            PipelineStageFlags shaderStage)
        {
            foreach (var texture in resources.Textures)
            {
                ...
                var depthRange =
                    DepthSubresourceRange(source.HasStencil);
                var destinationToTransfer = new ImageMemoryBarrier
                {
                    SType = StructureType.ImageMemoryBarrier,
                    DstAccessMask = AccessFlags.TransferWriteBit,
