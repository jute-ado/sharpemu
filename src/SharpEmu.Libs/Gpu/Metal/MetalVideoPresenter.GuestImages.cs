...
internal static partial class MetalVideoPresenter
{
    ...
    public static void RegisterKnownDisplayBuffer(GuestDisplayBuffer buffer)
    {
        if (buffer.Address == 0 ||
            buffer.Format == 0 ||
    ...
    public static void UnregisterKnownDisplayBuffer(ulong address)
    {
        if (address == 0)
        {
            return;
    ...
    private static bool HasReadyPresentation(long presentedSequence)
    {
        lock (_gate)
        {
            ...
            return _latestPresentation is { } latest &&
                latest.Sequence > presentedSequence &&
                (latest.Pixels is not null ||
                 (latest.TranslatedDraw is not null ||
                  latest.DrawKind != GuestDrawKind.None) &&
                 IsGuestWorkCompletedLocked(
