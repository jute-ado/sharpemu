...
internal sealed class MetalGuestGpuBackend : IGuestGpuBackend
{
    ...
    public bool SupportsTiledTextureUploads => false;
    ...
    public void RegisterKnownDisplayBuffer(GuestDisplayBuffer buffer) =>
        MetalVideoPresenter.RegisterKnownDisplayBuffer(buffer);

    public void UnregisterKnownDisplayBuffer(ulong address) =>
        MetalVideoPresenter.UnregisterKnownDisplayBuffer(address);
    ...
    public long SubmitGuestMemoryVisibilityAction(Action action, string debugName) =>
        MetalVideoPresenter.SubmitOrderedGuestAction(action, debugName);
