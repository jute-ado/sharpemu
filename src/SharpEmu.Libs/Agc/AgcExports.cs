...
public static partial class AgcExports
{
    ...
    private static readonly ArrayTextureUploadRetryTracker _arrayUploadRetryTracker = new();
    ...
    public static int DcbSetIndexCount(CpuContext ctx)
    {
        ...
        if (commandBufferAddress == 0 ||
            !TryAllocateCommandDwords(ctx, commandBufferAddress, 2, out var commandAddress) ||
            !TryWriteUInt32(ctx, commandAddress, Pm4(2, ItIndexBufferSize, 0)) ||
    ...
    public static int DcbCopyData(CpuContext ctx)
    {
        ...
        var stackAddress = ctx[CpuRegister.Rsp];
        if (!TryReadUInt64(ctx, stackAddress + sizeof(ulong), out var sourceAddressOrImmediate) ||
            !TryReadUInt64(ctx, stackAddress + (2 * sizeof(ulong)), out var itemSizeRaw) ||
        ...
        var control =
            ...
            ((source & 1u) << 30);
        if (!TryWriteUInt32(ctx, commandAddress, Pm4(6, ItCopyData, 0)) ||
            !TryWriteUInt32(ctx, commandAddress + 4, control) ||
    ...
    public static int DriverRegisterResource(CpuContext ctx)
    {
        ...
        if (resourceHandleAddress == 0 || resourceAddress == 0 || resourceSize == 0 ||
            !TryReadUInt32(ctx, ctx[CpuRegister.Rsp] + sizeof(ulong), out var flags) ||
            !TryReadGuestCString(
                ctx,
                nameAddress,
    ...
    private static bool ParseSubmittedDcb(
        ...
        bool tracePackets,
        int depth = 0,
        SubmittedDcbContinuation? continuation = null)
    {
        if (commandAddress == 0 ||
            dwordCount == 0 ||
        ...
        SubmittedDcbContinuation? continuation)
    {
        ...
        while (offset < dwordCount)
        {
            ...
            if (op == ItIndirectBuffer &&
                ...
                TryReadUInt32(ctx, currentAddress + 8, out var indirectAddressHigh) &&
                TryReadUInt32(ctx, currentAddress + 12, out var indirectControl))
            {
                var indirectAddress =
                    indirectAddressLow |
                ...
                length >= (register == RWaitMem32 ? 7u : 9u))
            {
                if (HandleSubmittedWaitRegMem(
                        ...
                        tracePackets, depth, continuation))
                {
        ...
        bool indexed)
    {
        ...
        // These modes operate on color-buffer metadata. The bound shaders are
        // command vehicles, not ordinary color draws. SharpEmu currently keeps
        // expanded host images without CMASK/FMASK/DCC state, so consuming the
        // operation is the correct no-op until that metadata is represented.
        if (TryGetCbMetadataColorMode(state.CxRegisters, out var cbMode))
        {
        ...
        var depthTarget = DecodeDepthTarget(state.CxRegisters, ctx.Memory);
        ...
        out string error)
    {
        ...
        if (!Gen5ShaderTranslator.TryCreateState(
            ...
            !Gen5ShaderScalarEvaluator.TryEvaluate(
                ...
                vertexRecordLimit: TryGetRequiredVertexRecordCount(
                    ctx,
                    state,
                    vertexCount,
                    indexed,
    ...
    internal static bool TryGetCbMetadataColorMode(
        IReadOnlyDictionary<uint, uint> registers,
        out uint mode) =>
        TryGetCbColorControlMode(registers, out mode) &&
        ...
        out GuestDrawTexture texture)
    {
            ...
            (isThreeDimensional && volumeUploadDepth > 1))
        {
            ...
            _arrayUploadRetryTracker.MarkUnsupported(descriptor.Address);
        }
        ...
        out ulong indirectDimsRetryAddress)
    {
        ...
        state.ShRegisters.TryGetValue(ComputeStartX, out var programmedBaseX);
        state.ShRegisters.TryGetValue(ComputeStartY, out var programmedBaseY);
        state.ShRegisters.TryGetValue(ComputeStartZ, out var programmedBaseZ);
        ...
        if (!TryResolveComputeDispatchTopology(
        ...
        var groupCountX = dispatch.GroupCountX;
        var groupCountY = dispatch.GroupCountY;
        var groupCountZ = dispatch.GroupCountZ;
        var baseGroupX = dispatch.BaseGroupX;
        var baseGroupY = dispatch.BaseGroupY;
        var baseGroupZ = dispatch.BaseGroupZ;
        var waveLaneCount = dispatch.WaveLaneCount;
    ...
    private static int AddIndirectPatchRegisters(CpuContext ctx, string registerSpace)
    {
        ...
        var newCount = (currentCount + registerCount) & 0x3FFFu;
        if (!TryWriteUInt32(ctx, commandAddress + 4, newCount))
        {
            return SetReturn(ctx, OrbisGen2Result.ORBIS_GEN2_ERROR_MEMORY_FAULT);
        }
    ...
    private static int DcbSetRegistersIndirect(CpuContext ctx, uint packetRegister, string registerSpace)
    {
        var commandBufferAddress = ctx[CpuRegister.Rdi];
        var registersAddress = ctx[CpuRegister.Rsi];
        var registerCount = (uint)ctx[CpuRegister.Rdx] & 0x3FFFu;
        if (commandBufferAddress == 0)
        {
            return ReturnPointer(ctx, 0);
        }
