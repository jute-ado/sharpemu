...
- dimension-correct RDNA image translation and Vulkan resource aliasing,
  ...
  and array layer before recording a full-extent Vulkan upload; Vulkan compute
  translates guest wave64 through native 32-lane subgroups instead of a
...
- a guest-thread scheduler with callback, synchronization, and lifecycle
  ...
  and immediate teardown signaling for interruptible guest waits; raw native
  guest workers are retired after each completed guest run so stale CLR
  reverse-P/Invoke transition state cannot cross run boundaries and
  nondeterministically terminate long-running titles
...
- guest virtual- and direct-memory query contracts with exact argument
  ...
  placement, flexible and direct named-map pointer and 32-byte length validation
  with names retained in virtual-query results across protection and fixed-remap
  splits while displaced-region names are discarded, exact subrange renaming
...
- audio session teardown that disposes host AudioOut streams, clears the
  ...
  multi-port batch only once; the first accepted non-silent PCM buffer emits one
  stable session marker for portable game-progression tests; AudioOut2
...
- offline user-service defaults with the size-aware two-argument PS5 game-preset
  ...
  commercial titles
- Gen5 Share feature-permission handling that accepts every nonzero feature
  mask and returns the Share-specific invalid-parameter result for an empty
  mask
...
- complete six-register and first-stack-argument SysV diagnostics for resolved
  ...
  evidence; expected adaptive-mutex self-lock, APR missing-file, first-run
  save-data mount, existing-directory creation, and disconnected-controller
  probe results stay out of warning budgets while mismatched NID/result pairs
  remain visible
...
- AGC indirect-draw command builders that decode full 64-bit modifiers into
  ...
  Gen5 conditional command-buffer branch packet with mode-aware then/else
  execution and masked 64-bit comparisons, exposes an exact inactive
  GPU-capture status, and lazily bootstraps optional resource registration for
  valid pre-initialization calls, while resource handles, command queues,
  register-default allocations, and GPU waits now share one stable state across
  transient CPU memory-access wrappers
