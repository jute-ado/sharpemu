...
| SILENT HILL: The Short Message | PPSA10112 | Loads six modules and sustains the execution-survival window without a CPU trap. The Gen5 conditional command-buffer branch builder now emits the exact 14-dword packet, and submission selects its then/else child using mode-aware masked 64-bit comparisons. A fresh 65-second diagnostic resolved the previously missing branch import and removed the earlier managed/native-call fatal marker without a device loss, fence timeout, or presenter failure. Remaining diagnostics include an unresolved Share feature-permission query, expected filesystem/controller results, and producerless GPU waits. No gameplay is validated yet. |
