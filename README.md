...
| The Last of Us Part I | PPSA03396 | Loads the main image and `libc.prx`. Gen5 VRR-status privilege and inactive-status event registration now resolve with validated library identities. Correct Orbis `EEXIST` encoding removes the subsequent FIOS misinterpretation as `SCE_KERNEL_ERROR_EINTR`, while the AGC driver's inactive capture-status query removes the high-frequency unresolved polling import. Filesystem reachability now resolves through the guest sandbox and correctly reports the title's absent `/app0/manifest.txt`. The Net MAC-address query and its 18-byte text conversion now resolve in sequence using a stable virtual adapter identity. In a targeted 438 MiB flexible-memory diagnostic the title sustains the full 90-second execution window without a CPU trap, unimplemented-function stop, or memory fault; it reaches at least 2.4 million import dispatches, mounts the main and world archives, and begins switching into the core world. This memory configuration is diagnostic rather than the production default. No rendered frame or gameplay is validated. |
