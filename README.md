...
- dimension-correct RDNA image translation and Vulkan resource aliasing,
  ...
  Gen5 R16 and RGBA32 unsigned and signed integer formats with matching shader
  output types instead of dropping those passes
...
- AGC indirect-draw command builders that decode full 64-bit modifiers into
  ...
  registration for valid pre-initialization calls, while resource handles,
  command queues, register-default allocations, and GPU waits now share one
  stable state across transient CPU memory-access wrappers
