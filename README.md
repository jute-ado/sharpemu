...
| The Last of Us Part I | PPSA03396 | Loads the main image and `libc.prx`. Gen5 VRR-status privilege and inactive-status event registration now resolve with validated library identities. In a targeted 438 MiB flexible-memory diagnostic this clears both VideoOut initialization assertions, exceeds 1.5 million import dispatches, mounts the main game archives, and reaches world/content setup before FIOS halts on `SCE_KERNEL_ERROR_EINTR` while creating `/download0/http-cache/`. This memory configuration is diagnostic rather than the production default. No rendered frame or gameplay is validated. |
