...
public sealed unsafe class Sse4aPosixSignalRecoveryTests
{
    ...
    private static readonly FieldInfo HostMemory = typeof(DirectExecutionBackend).GetField(
        "_hostMemory",
    ...
    public void RecoveryDeclinesWhenNoXmmStateWasBridged()
    {
        ...
        try
        {
            ...
            var backend = CreateProbeBackend();
    ...
    /// instance with only the host-memory service used by instruction probing.
    /// </summary>
    ...
    private static object CreateProbeBackend()
    {
    ...
    private static nint AllocateProbeVisibleCode(ReadOnlySpan<byte> instructions)
    {
        ...
        var mapping = HostPlatform.Current.Memory.Allocate(
            0,
            size,
            HostPageProtection.ReadWrite);
        Assert.NotEqual(0UL, mapping);

        instructions.CopyTo(new Span<byte>((void*)mapping, checked((int)size)));
        return unchecked((nint)mapping);
    }

    private static void FreeProbeVisibleCode(nint mapping)
    {
        Assert.True(HostPlatform.Current.Memory.Free(unchecked((ulong)mapping)));
    }
}
