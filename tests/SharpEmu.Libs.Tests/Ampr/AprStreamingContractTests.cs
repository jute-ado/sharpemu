...
public sealed class AprStreamingContractTests
{
    ...
    public void ResolveStatAndReadFile_UsesSharedAprFileId()
    {
        ...
        var mountPoint = "/apr-test-" + Guid.NewGuid().ToString("N");
        var hostRoot = Path.Combine(Path.GetTempPath(), "SharpEmu.Tests", Guid.NewGuid().ToString("N"));
        var hostPath = Path.Combine(hostRoot, "stream.bin");
        var guestPath = mountPoint + "/stream.bin";
