...
public sealed class SaveDataExportsTests : IDisposable
{
    ...
    private string SlotDir =>
        Path.Combine(_root, UserId.ToString(), TitleId, DirName);
    ...
    public void SetParam_ThenGetParam_RoundTripsThroughMetadata()
    {
        ...
        Assert.Equal(
            0,
            SaveDataExports.SaveDataSetParam(
                Reg(
                    rdi: MountPointStr,
                    rdx: ParamStruct,
                    rcx: 0x530)));

        Assert.True(File.Exists(Path.Combine(SlotDir, "sce_sys", "param.bin")));
        ...
        Assert.Equal(
            0,
            SaveDataExports.SaveDataGetParam(
                Reg(
                    rdi: MountPointStr,
                    rdx: ParamStruct,
                    rcx: 0x530)));
    ...
    public void Delete_RemovesTheSlotDirectory()
    {
        ...
        BinaryPrimitives.WriteUInt64LittleEndian(del[0x10..], DirNamePtr);
