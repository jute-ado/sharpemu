...
public sealed class AgcWaitRegMemTests
{
    ...
    [Fact]
    public void DcbWaitRegMemStandardOperation_EmitsNativeWaitPacket()
    {
        var memory = CreateMemory(out var ctx);
        ...
        Assert.Equal((int)OrbisGen2Result.ORBIS_GEN2_OK, AgcExports.DcbWaitRegMem(ctx));
        Assert.Equal(0xC005_3C00u, ReadUInt32(memory, PacketAddress));
        Assert.Equal(0x0000_0106u, ReadUInt32(memory, PacketAddress + 4));
        Assert.Equal(0x0000_0C07u, ReadUInt32(memory, PacketAddress + 8));
        Assert.Equal(1u, ReadUInt32(memory, PacketAddress + 12));
        Assert.Equal(0x5566_7788u, ReadUInt32(memory, PacketAddress + 16));
        Assert.Equal(0xEEFF_0011u, ReadUInt32(memory, PacketAddress + 20));
        Assert.Equal(0x14u, ReadUInt32(memory, PacketAddress + 24));
        Assert.Equal(PacketAddress + 28, ReadUInt64(memory, CommandBufferAddress + 0x10));
    }
