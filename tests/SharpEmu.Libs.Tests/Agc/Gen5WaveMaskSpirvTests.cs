...
public sealed class Gen5WaveMaskSpirvTests
{
    ...
    public void WaveMaskPredicate_IsTestedAtCurrentLaneBit()
    {
        ...
        // The lane bit is either the 64-bit constant 1 in single-lane mode or
        // `1UL << lane` under this fork's wave64/subgroup emulation. The
        // whole-word bug emitted `mask != 0` with no such mask. Require the
        // lane-bit AND to be present in either representation.
        Assert.True(
    ...
    // True when the module contains an OpBitwiseAnd whose operand is either the
    // constant lane bit 1 or a dynamic `1UL << lane` derived from it.
    private static bool ContainsLaneBitMaskedWaveTest(byte[] spirv)
    {
        ...
        var laneBitIds = new HashSet<uint>(laneBitConstIds);
        foreach (var (op, wordCount, offset) in EnumerateInstructions(spirv))
        {
            // OpShiftLeftLogical = 196. CurrentLaneBit builds 1UL << lane for
            // wave64/subgroup-aware compute shaders.
            ...
            var resultId = ReadWord(spirv, offset + 8);
            var baseOperand = ReadWord(spirv, offset + 12);
            if (laneBitConstIds.Contains(baseOperand))
            {
                laneBitIds.Add(resultId);
            }
        }

        foreach (var (op, wordCount, offset) in EnumerateInstructions(spirv))
        {
            // OpSelect = 169. Subgroup mode selects the shifted bit for valid
            // RDNA lanes and zero for host-only lanes.
            ...
            var resultId = ReadWord(spirv, offset + 8);
            var trueValue = ReadWord(spirv, offset + 16);
            var falseValue = ReadWord(spirv, offset + 20);
            if (laneBitIds.Contains(trueValue) || laneBitIds.Contains(falseValue))
            {
                laneBitIds.Add(resultId);
            }
        }
        ...
        foreach (var (op, wordCount, offset) in EnumerateInstructions(spirv))
        {
            ...
            if (laneBitIds.Contains(operand0) || laneBitIds.Contains(operand1))
            {
                return true;
            }
        }
