...
public sealed class GuestMemoryAllocatorTests
{
    ...
    private sealed class RecordingHostMemory : IHostMemory, IDisposable
    {
        ...
        {
            // PhysicalVirtualMemory reserves mappings at the Windows allocation
            // granularity even when individual protection runs are page-sized.
            _allocation = System.Runtime.InteropServices.Marshal.AllocHGlobal(checked((nint)(size + 0xFFFF)));
            Address = (unchecked((ulong)_allocation) + 0xFFFF) & ~0xFFFFUL;
        }
